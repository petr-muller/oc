...
func New(f kcmdutil.Factory, streams genericiooptions.IOStreams) *cobra.Command {
	...
	flags.StringVar(&o.mockCvPath, "mock-clusterversion", "", "Path to a YAML ClusterVersion object to use for testing (will be removed later). Files in the same directory with the same name and suffixes -co.yaml, -mcp.yaml, -mc.yaml, and -node.yaml are required.")
...
type options struct {
	...
	mockMachineConfigPoolsPath string
	mockMachineConfigsPath     string
	mockNodesPath              string
	mockClusterVersion         *configv1.ClusterVersion
	mockClusterOperators       *configv1.ClusterOperatorList
	mockMachineConfigPools     *machineconfigv1.MachineConfigPoolList
	mockMachineConfigs         *machineconfigv1.MachineConfigList
...
func (o *options) Complete(f kcmdutil.Factory, cmd *cobra.Command, args []string) error {
	...
	cvSuffix := "-cv.yaml"
	if o.mockCvPath != "" {
		o.mockOperatorsPath = strings.Replace(o.mockCvPath, cvSuffix, "-co.yaml", 1)
		o.mockMachineConfigPoolsPath = strings.Replace(o.mockCvPath, cvSuffix, "-mcp.yaml", 1)
		o.mockMachineConfigsPath = strings.Replace(o.mockCvPath, cvSuffix, "-mc.yaml", 1)
		o.mockNodesPath = strings.Replace(o.mockCvPath, cvSuffix, "-node.yaml", 1)
	}
...
func (o *options) Run(ctx context.Context) error {
	...
	if pools = o.mockMachineConfigPools; pools == nil {
		var err error
	...
	if allNodes = o.mockNodes; allNodes == nil {
		var err error
	...
	if machineConfigs = o.mockMachineConfigs; machineConfigs == nil {
		for _, node := range allNodes.Items {
			var err error
			machineConfigs, err = appendMachineConfig(ctx, o.MachineConfigClient, machineConfigs, node.Annotations[machineconfigconst.CurrentMachineConfigAnnotationKey])
			if err != nil {
				return err
			}
			machineConfigs, err = appendMachineConfig(ctx, o.MachineConfigClient, machineConfigs, node.Annotations[machineconfigconst.DesiredMachineConfigAnnotationKey])
			if err != nil {
				return err
			}
		}
