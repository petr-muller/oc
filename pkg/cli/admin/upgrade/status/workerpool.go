...
func appendMachineConfig(ctx context.Context, client mcfgv1client.Interface, machineConfigList *mcfgv1.MachineConfigList, machineConfigName string) (*mcfgv1.MachineConfigList, error) {
	if machineConfigList == nil {
		machineConfigList = &mcfgv1.MachineConfigList{}
	}
	found := false
	...
	if !found {
		machineConfig, err := client.MachineconfigurationV1().MachineConfigs().Get(ctx, machineConfigName, v1.GetOptions{})
		if err != nil {
			return machineConfigList, err
		} else {
			machineConfigList.Items = append(machineConfigList.Items, *machineConfig)
		}
	}
	return machineConfigList, nil
}
...
func separateMasterAndWorkerPools(poolList *mcfgv1.MachineConfigPoolList) (mcfgv1.MachineConfigPool, []mcfgv1.MachineConfigPool) {
	...
	return mcfgv1.MachineConfigPool{}, []mcfgv1.MachineConfigPool{}
}

func selectNodesFromPool(pool mcfgv1.MachineConfigPool, nodes []corev1.Node) ([]corev1.Node, error) {
	var res []corev1.Node
...
func assessNodesStatus(cv *configv1.ClusterVersion, pool mcfgv1.MachineConfigPool, nodes []corev1.Node, machineConfigs []mcfgv1.MachineConfig) ([]nodeDisplayData, []updateInsight) {
	...
	for _, node := range nodes {
		...
		phase := calculatePhase(pool, node, isUpdating, isUpdated, isDegraded)
		...
		if isUnavailable && !isUpdating {
			...
			message = "Node is unavailable. Please check its status." // TODO(dhurta/OTA): Bubble up the exact reason.
			estimate = "?"
			if isUpdated {
				estimate = "-"
			}
...
func isNodeDraining(node corev1.Node, isUpdating bool) bool {
	...
	if isUpdating &&
		mcdUpdatingStateToPhase(node.Annotations[mcfgconst.MachineConfigDaemonStateAnnotationKey]) == phaseStateUpdated {
...
func calculatePhase(pool mcfgv1.MachineConfigPool, node corev1.Node, isUpdating, isUpdated, isDegraded bool) nodePhase {
	var phase nodePhase
	if isUpdating {
		if isNodeDraining(node, isUpdating) {
...
func nodeInsights(pool mcfgv1.MachineConfigPool, node corev1.Node, reason string, isUnavailable, isUpdating, isDegraded bool) []updateInsight {
	...
	if isDegraded {
		// TODO(dhurta/OTA): What are all the impacts of a degraded state reported by the MCD?
		// TODO(dhurta/OTA): Is stalled the appropriate default value?
		insights = append(insights, updateInsight{
			startedAt: time.Time{},
			scope: updateInsightScope{
				scopeType: scope,
...
func (d *poolDisplayData) WritePool(f io.Writer) error {
	return workerPoolStatusTemplate.Execute(f, d)
}
...
func (pool *poolDisplayData) WriteNodes(w io.Writer) {
	...
	tabw.Flush()
	if total > 0 {
		fmt.Fprintf(w, "Omitted additional %d Total, %d Completed, %d Available, %d Progressing, %d Outdated, %d Draining, %d Excluded, and %d Degraded nodes. Pass along --details to see all information.\n", total, completed, available, progressing, outdated, draining, excluded, degraded)
	}
}
